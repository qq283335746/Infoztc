...
var AddAdvertisement = {
    ...
    CbbContentType: function (clientId) {
        ...
        $.ajax({
            url: "/ScriptServices/AdminService.asmx/GetJsonForContentTypeByTypeCode",
            type: "post",
    ...
    DisplayEle: function (text, t) {
        ...
        switch (text) {
            case "跳转至商品":
                currParent.find("[name=Url]:first").parent().prev().text('商品ID：');
                currParent.find("[name=Url]:first").parent().parent().show();
                break;
            case "跳转至外部网页":
                currParent.find("[name=Url]:first").parent().prev().text('Url：');
                currParent.find("[name=Url]:first").parent().parent().show();
                break;
            case "图文":
