...
namespace TygaSoft.Web.Admin.Lottery
{
    public partial class ListLottery : System.Web.UI.Page
    {
        ...
        private void GetSearchItem()
        {
            ...
            if (!string.IsNullOrWhiteSpace(hnqs))
            {
                if (parms == null) parms = new ParamsHelper();

                sqlWhere += "and HNQS like @HNQS ";
                SqlParameter parm = new SqlParameter("@HNQS", SqlDbType.VarChar, 30);
                parm.Value = "%" + hnqs + "%";
                parms.Add(parm);
