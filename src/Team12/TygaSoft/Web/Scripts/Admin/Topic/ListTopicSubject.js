...
var ListTopicSubject = {
    ...
    Search: function () {
        window.location = "?title=" + $.trim($("[id$=txtTitle]").val()) + "&content=" + $.trim($("[id$=txtContent]").val()) + "";
    },
    Add: function () {
        window.location = "ytopic.html?isTop=False";
    },
    Edit: function () {
        ...
        window.location = "ytopic.html?Id=" + rows[0].f0 + "&isTop=" + rows[0].f7;
    },
    Del: function () {
        try {
            var rows = $('#dgT').datagrid("getSelections");
            ...
            $.messager.confirm('温馨提醒', '确定要删除吗？', function (r) {
                if (r) {
                    $.ajax({
                        url: "/ScriptServices/AdminService.asmx/DelTopic",
                        type: "post",
    ...
    UpdateIsTop: function (id, isTop) {
        $.ajax({
            url: "../../ScriptServices/AdminService.asmx/SaveTopicIsTop",
            type: "post",
