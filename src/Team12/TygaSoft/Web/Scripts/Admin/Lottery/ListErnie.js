...
var ListErnie = {
    ...
    Search: function () {
        window.location = "?startTime=" + $.trim($("[id$=txtStartTime]").val()) + "&endTime=" + $.trim($("[id$=txtEndTime]").val()) + "";
    },
    Add: function () {
        window.location = "yernie.html";
    },
    Edit: function () {
        ...
        window.location = "yernie.html?Id=" + rows[0].f0;
    },
    Del: function () {
        try {
            var rows = $('#dgT').datagrid("getSelections");
            ...
            $.messager.confirm('温馨提醒', '确定要删除吗？', function (r) {
                if (r) {
                    $.ajax({
                        url: "/ScriptServices/AdminService.asmx/DelErnie",
                        type: "post",
