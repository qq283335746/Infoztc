...
var ListTopicComment = {
    ...
    Search: function () {
        window.location = "?tsId=" + $("#tsId").val() + "&content=" + $.trim($("[id$=txtContent]").val()) + "";
    },
    Add: function () {
        window.location = "atopic.html?tsId=" + $("#tsId").val() + "&isTop=False";
    },
    Edit: function () {
        ...
        window.location = "atopic.html?Id=" + rows[0].f0 + "&isTop=" + rows[0].f6;
    },
    Del: function () {
        try {
            var rows = $('#dgT').datagrid("getSelections");
            ...
            $.messager.confirm('温馨提醒', '确定要删除吗？', function (r) {
                if (r) {
                    $.ajax({
                        url: "/ScriptServices/AdminService.asmx/DelTopicComment",
                        type: "post",
    ...
    UpdateIsTop: function (id, isTop) {
        $.ajax({
            url: "../../ScriptServices/AdminService.asmx/SaveTopicCommentIsTop",
            type: "post",
            data: '{model:{Id:"' + id + '",IsTop:"' + isTop + '"}}',
            contentType: "application/json; charset=utf-8",
            beforeSend: function () {
                $("#dlgWaiting").dialog('open');
            },
