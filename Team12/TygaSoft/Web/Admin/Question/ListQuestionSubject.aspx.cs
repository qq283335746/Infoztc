...
{
    public partial class ListQuestionSubject : System.Web.UI.Page
    {
        ...
        private void GetSearchItem()
        {
            ...
            if (!string.IsNullOrWhiteSpace(name))
            {
                ...
                sqlWhere += "and QuestionContent like @QuestionContent ";
                SqlParameter parm = new SqlParameter("@QuestionContent", SqlDbType.NVarChar, 512);
                parm.Value = "%" + name + "%";
                parms.Add(parm);
            }

            Guid gId = Guid.Empty;
            if (Guid.TryParse(qbId, out gId))
            {
                if (parms == null) parms = new ParamsHelper();

                sqlWhere += "and QuestionBankId = @QuestionBankId ";
                SqlParameter parm = new SqlParameter("@QuestionBankId", SqlDbType.UniqueIdentifier);
                parm.Value = gId;
                parms.Add(parm);
            }
        }
