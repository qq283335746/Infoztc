...
{
    /// <summary>
    /// HandlerSizePicture 的摘要说明
    /// </summary>
    public class HandlerSizePicture : IHttpHandler
    {
        ...
        private void OnUpload(HttpContext context)
        {
            ...
            {
                int effect = 0;
                bool isCreateThumbnail = ConfigHelper.GetValueByKey("IsCteateSizePictureThumbnail") == "1";
                ...
                foreach (string item in files.AllKeys)
                {
                    ...
                    string[] paths = ufh.Upload(files[item], "SizePictures", isCreateThumbnail);
                    SizePicture ppBll = new SizePicture();
                    SizePictureInfo ppModel = new SizePictureInfo();
