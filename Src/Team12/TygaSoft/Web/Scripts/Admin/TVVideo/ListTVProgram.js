...
var ListTVProgram = {
    ...
    Search: function () {
        window.location = "?name=" + $.trim($("[id$=txtName]").val()) + "&qsId=" + $("#hId").val() + "";
    },
    Add: function () {
        window.location = "atv.html?hwid=" + $("#hId").val() + "";
    },
    Edit: function () {
        ...
        window.location = "atv.html?Id=" + rows[0].f0;
    },
    Del: function () {
        try {
            var rows = $('#dgT').datagrid("getSelections");
            ...
            $.messager.confirm('温馨提醒', '确定要删除吗？', function (r) {
                if (r) {
                    $.ajax({
                        url: "/ScriptServices/AdminService.asmx/DelTVProgram",
                        type: "post",
