...
var ListLottery = {
    ...
    Search: function () {
        window.location = "?qs=" + $.trim($("[id$=txtQS]").val()) + "&hnqs=" + $.trim($("[id$=txtHNQS]").val()) + "";
    },
    Add: function () {
        window.location = "ylottery.html";
    },
    Edit: function () {
        ...
        window.location = "ylottery.html?Id=" + rows[0].f0;
    },
    Del: function () {
        try {
            var rows = $('#dgT').datagrid("getSelections");
            ...
            $.messager.confirm('温馨提醒', '确定要删除吗？', function (r) {
                if (r) {
                    $.ajax({
                        url: "/ScriptServices/AdminService.asmx/DelLottery",
                        type: "post",
