...
var ListActivityPrize = {
    ...
    Search: function () {
        window.location = "?asId=" + $("#asId").val() + "&name=" + $.trim($("[id$=txtName]").val()) + "";
    },
    Add: function () {
        window.location = "ygactivity.html?asId=" + $("#asId").val();
    },
    Edit: function () {
        ...
        window.location = "ygactivity.html?Id=" + rows[0].f0;
    },
    Del: function () {
        try {
            var rows = $('#dgT').datagrid("getSelections");
            ...
            $.messager.confirm('温馨提醒', '确定要删除吗？', function (r) {
                if (r) {
                    $.ajax({
                        url: "/ScriptServices/AdminService.asmx/DelActivityPrize",
                        type: "post",
    ...
    BindPrizeHref: function(id, isPrize) {
        if (isPrize == "True") {
